...
# Build/install the vast-core python package.
...
# This can be set with the VAST_CMAKE_BUILD_DIR env var.
#
# By default, this will use the llvm-project submodule included with VAST.
# This can be overridden with the VAST_LLVM_DIR env var.
...
class CMakeBuild(build_py):
  ...
  def run(self):
    ...
    vast_dir = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "..", ".."))
    cmake_build_dir = os.getenv("VAST_CMAKE_BUILD_DIR")
    if not cmake_build_dir:
      cmake_build_dir = os.path.join(vast_dir, "build")
    cmake_install_dir = os.path.join(cmake_build_dir, "..", "install")
    llvm_dir = os.getenv("VAST_LLVM_DIR")
    if not llvm_dir:
      llvm_dir = os.path.join(vast_dir, "llvm")
    cmake_args = [
        ...
        "-DLLVM_EXTERNAL_PROJECTS=vast",
        "-DLLVM_EXTERNAL_VAST_SOURCE_DIR={}".format(vast_dir),
        "-DLLVM_TARGETS_TO_BUILD=host",
        "-DMLIR_ENABLE_BINDINGS_PYTHON=ON",
        "-DVAST_BINDINGS_PYTHON_ENABLED=ON",
    ]
...
setup(
    ...
    author_email="henrich.lauko@trailofbits.com",
    description="VAST Core",
    long_description="",
    include_package_data=True,
    ext_modules=[
        CMakeExtension("mlir._mlir_libs._mlir"),
        CMakeExtension("mlir._mlir_libs._vast"),
    ],
    cmdclass={
        "build": CustomBuild,
